...
class PriorityAdvancedSearchSystem(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if order == ORDERMAYORMINOR:
            queryset = models.Requisition.objects.all().filter(affectedSystem=system, status=status).order_by(
                'priority')
...
class PriorityAdvancedSearchTechnician(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if order == ORDERMAYORMINOR:
            queryset = models.Requisition.objects.all().filter(assignedTechnician=technician, status=status).order_by(
                'priority')
